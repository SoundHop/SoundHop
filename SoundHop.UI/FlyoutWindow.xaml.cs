    ...
    public sealed partial class FlyoutWindow : Window
    {
        ...
        private void ShowWithAnimation()
        {
            ...
            DispatcherQueue.TryEnqueue(() =>
            {
                ...
                SoundHop.UI.Helpers.FlyoutAnimationHelper.BeginEntranceAnimation(this.Content, _lastTaskbarState.Location, () =>
                {
                    _isAnimating = false;
                    SoundHop.Core.Interop.User32.SetForegroundWindow(_hWnd);
                });
            });
        }
        ...
        {
            _isAnimating = true;
            SoundHop.UI.Helpers.FlyoutAnimationHelper.BeginExitAnimation(this.Content, _lastTaskbarState.Location, () =>
            {
                _isAnimating = false;
                ShowWindow(_hWnd, 0); // SW_HIDE
                SoundHop.UI.Helpers.FlyoutAnimationHelper.ResetVisual(this.Content);
            });
        }

        private void PositionFlyout(SoundHop.Core.Interop.WindowsTaskbar.State taskbar)
        {
            ...
            if (taskbar.IsAutoHideEnabled)
            {
                ...
                {
                    case SoundHop.Core.Interop.WindowsTaskbar.Position.Left:
                        if (workArea.Left < tb.Right) adjustedLeft = tb.Right;
                        break;
                    case SoundHop.Core.Interop.WindowsTaskbar.Position.Right:
                        if (workArea.Right > tb.Left) adjustedRight = tb.Left;
                        break;
                    case SoundHop.Core.Interop.WindowsTaskbar.Position.Top:
                        if (workArea.Top < tb.Bottom) adjustedTop = tb.Bottom;
                        break;
                    case SoundHop.Core.Interop.WindowsTaskbar.Position.Bottom:
            ...
            SoundHop.Core.Interop.User32.GetCursorPos(out var cursorPos);
